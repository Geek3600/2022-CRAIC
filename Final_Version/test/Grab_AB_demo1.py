...
def deal_with_apriltag_info(apriltag_bunble_info, option="coordinate") -> string:
...
# 微调小车位置
def adjust_car_pos(x_adjust, y_adjust, apriltag_label) -> string:
...
def check_numbers_of_blocks() -> string:
...
def navigation():
    ...
    # 发送导航指令    
    send_data_for_navi = "B1GotoTarget;1#" 
...
移动抓取：
    ...
    2.监听导航结果,是否到位
    3.视觉抓取(循环)
        1.检测暂存区方块数量（假设每次抓的都对，只出现抓空的情况）
        2.机械臂去观察位(armmove)
        3.查看用于微调小车位置的二维码位置()
        4.微调小车的位置（adjustmove）
        5.再次查看用于抓取的二维码的位置
        6.去抓取位(armmove)
        7.吸气（pump operation）
        8.回到观察位(armmove)
